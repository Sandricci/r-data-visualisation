...
server <- function(input, output) {
  ...
  output$distplot <- renderPlot({
    ...
    if(input$density) {
      hist(var, main="Histogram with Density Plot", xlab = input$outcome, yaxt = "n", ylab = NULL, xaxs ="i", freq = F, breaks = input$bins)
      d <- density(var)
      ...
      h <- hist(subset[,input$outcome], main="Histogram", xlab=input$outcome, yaxt = "n", ylab = NULL, xaxs ="i")
      # set normal curve
